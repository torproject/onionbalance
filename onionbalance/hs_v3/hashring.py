...
class HSV3HashRing(object):
    def __init__(self):
        self.hsdir_n_replicas = params.HSDIR_N_REPLICAS
        self.hsdir_spread_store = params.HSDIR_SPREAD_STORE

    def _time_between_tp_and_srv(self, valid_after):
        """
    ...
    def _get_srv_and_time_period(self, is_first_descriptor):
        """
        ...
        if is_first_descriptor:
            if self._time_between_tp_and_srv(valid_after):
    ...
    def _get_hash_ring_for_descriptor(self, is_first_descriptor):
        """
        ...
        srv, time_period_num = self._get_srv_and_time_period(is_first_descriptor)
...
#            logger.debug("%s: Node: %s,  index: %s", is_first_descriptor, node.get_hex_fingerprint(), hsdir_index.hex())
    ...
    def _get_hidden_service_index(self, blinded_pubkey, replica_num, is_first_descriptor):
        """
        ...
        _, time_period_num = self._get_srv_and_time_period(is_first_descriptor)
    ...
    def get_responsible_hsdirs(self, blinded_pubkey, is_first_descriptor):
        """
        ...
        node_hash_ring = self._get_hash_ring_for_descriptor(is_first_descriptor)
        ...
        for replica_num in range(1, self.hsdir_n_replicas+1):
            # The HSDirs that we are gonna store this replica in
            replica_store_hsdirs = []

            hidden_service_index = self._get_hidden_service_index(blinded_pubkey, replica_num, is_first_descriptor)
            ...
            while len(replica_store_hsdirs) < self.hsdir_spread_store:
                try:
        ...
        if my_onionbalance.is_testnet:
            ...
            assert(len(responsible_hsdirs) <= self.hsdir_n_replicas*self.hsdir_spread_store)
        else:
            assert(len(responsible_hsdirs) == self.hsdir_n_replicas*self.hsdir_spread_store)
